...
def export_Info(tabla, desde, hasta, type):
    ...
    try:
        ...
        if re.search(r"42S02", str(e)):
            show_messagebox_nok(f"La Habitación No Posee Información", 40)
#####################################################################################################################################################
...
def set_actual_date(type):
    ...
    if type=="Reports":
        ...
        ToEntryOcup.delete(0, 'end')
        ToEntryOcup.insert(0, ActualDate)
...
def export_time(time, type):
    ...
    if type=="Reports":
        ...
        FromEntryOcup.delete(0, 'end')
        FromEntryOcup.insert(0, ar_time[time])
        ToEntryOcup.delete(0, 'end')
        ToEntryOcup.insert(0, ar_time[2])
        export(type)
    elif type=="Ocup":
...
def export(type):
    if type=="Reports":
        ...
        table = "Ocupaciones"
        since = FromEntryOcup.get()
        till = ToEntryOcup.get()
    elif type=="Habs":
...
def show_Ocup():
    FrameExportaciones.place_forget()
    FrameExportOcup.place(relx=0.5, rely=0.5, anchor='center')
    GoBackButton.place(relx=0, x=0, y=0, anchor='nw')
...
FrameExportOcup = tk.Frame(root, bg= '#505050')
...
#Open Ocupations
OpenFrameOcup = ctk.CTkButton(FrameExportaciones, text="Exportar Ocupaciones", command=show_Ocup)
OpenFrameOcup.grid(row=2, column=1, padx=10, pady=20)

#To Frame Ocupations
LabelOcup = tk.Label(FrameExportOcup, text="Exportador Tabla Ocupaciones", bg= '#505050', fg='white')
LabelOcup.grid(row=0, columnspan=3, pady=5)

FromLabelOcup = tk.Label(FrameExportOcup, text="Desde:", bg= '#505050', fg='white')
FromLabelOcup.grid(row=1, column=0)
FromEntryOcup = tk.Entry(FrameExportOcup, width=12, font= 10)
FromEntryOcup.grid(row=1, column=1)

ToLabelOcup = tk.Label(FrameExportOcup, text="Hasta:", bg='#505050', fg='white')
ToLabelOcup.grid(row=2, column=0)
ToEntryOcup = tk.Entry(FrameExportOcup, width=12, font= 10)
ToEntryOcup.grid(row=2, column=1)

#Aplicate Actual Date
ActualDateButtonOcup = ctk.CTkButton(FrameExportOcup, text="Fecha actual", border_width=1, command=lambda:set_actual_date(str("Ocup")))
ActualDateButtonOcup.grid(row=2, column=2, padx=5)

#Export day
ExportLastDayOcup = ctk.CTkButton(FrameExportOcup, text="Exportar Último Dia", command=lambda:export_time(0, (str("Ocup"))))
ExportLastDayOcup.grid(row=1, column=4, padx=5, pady=2)

#Export month
ExportLastMonthOcup = ctk.CTkButton(FrameExportOcup, text="Exportar Último Mes", command=lambda:export_time(1, (str("Ocup"))))
ExportLastMonthOcup.grid(row=2, column=4, padx=5, pady=2)

#Export
ExportButtonOcup = ctk.CTkButton(FrameExportOcup, text="Exportar", command=lambda:export(str("Ocup")))
ExportButtonOcup.grid(row=3, column=1, columnspan=1, pady=(10, 0), sticky="nsew")
...
def adjust_size(width, height):
    ...
    OpenFrameOcup.configure(font=start_button_font)
    LabelOcup.config(font=title_font)
    ToLabelOcup.configure(font=text_font)
    ToEntryOcup.configure(font=text_font)
    FromLabelOcup.configure(font=text_font)
    FromEntryOcup.configure(font=text_font)
    ActualDateButtonOcup.configure(font=button_font)
    ExportButtonOcup.configure(font=button_font)
    ExportLastDayOcup.configure(font=button_font)
    ExportLastMonthOcup.configure(font=button_font)
