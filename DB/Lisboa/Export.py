...
def export_Info(tabla, desde, hasta, type):
    ...
    try:
        if type=="ALARMHISTORY":
            query = f"SELECT Al_Start_Time, Al_Message FROM {tabla} WHERE Time_Stamp >= '{desde}' AND Time_Stamp <= '{hasta} 23:59:59.000000'"
            ...
            CantidadValores = 5
        elif type =="EVENTHISTORY":
            query = f"SELECT Ev_Time, Ev_Message FROM {tabla} WHERE Time_Stamp >= '{desde}' AND Time_Stamp <= '{hasta} 23:59:59.000000'"
