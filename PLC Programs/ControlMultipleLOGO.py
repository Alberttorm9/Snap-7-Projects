...
if accion == 1:
    ...
    if accion==("Si" or "si" or "SI"):
        ...
        for i in range(10):
            ...
            try:
                ...
                if not (plc.read("V10.3")):
                    plc.write("V10.1", 1)
                    time.sleep(0.5)
                    plc.write("V10.1", 0)
                    print(f"PLC {i+101} Abriendo")
                    numero_abiertos_cerrados += 1
                else:
                    print(f"PLC{i+101} Ya abierto")
                    numero_abiertos_cerrados += 1
                plc.disconnect()
            except Exception as err:
                print(f"Error de conexión al PLC {i+101}: {err}")
                print(f"PLC {i+101} Error")
                numero_fallidos += 1

    print(f"Fallaron: {numero_fallidos}")
    print(f"Ya abiertos: {numero_abiertos_cerrados}")
    ...
    if accion==("Si" or "si" or "SI"):
        ...
        for i in range(10):
            ...
            try:
                plc.connect(f'192.168.30.{i+101}', 0, 1)
                if not (plc.read("V12.2")):
                    plc.write("V10.1", 1)
                    time.sleep(0.5)
                    plc.write("V10.1", 0)
                    print(f"PLC {i+101} Conectado")
                    numero_abiertos_cerrados += 1
                else:
                    print(f"PLC{i+101} Ya abierto")
                    numero_abiertos_cerrados += 1
